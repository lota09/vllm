...
| 사실 | vLLM 에 미치는 영향 |
|:---|:---|
| **실사용 VRAM ≈ 63.4 GiB** (`nvidia-smi` 의 65536 MiB 는 라벨) | [`quantization.md`](quantization.md) §1.1 산수 참조. **clamp 덕에 초과 할당은 깨끗한 OOM** — 실험해도 카드는 안 죽는다 |
...
| llama.cpp | vLLM | 주의 |
|:---|:---|:---|
| `-m model.gguf` | `vllm serve <HF리포 또는 디렉터리>` | **파일이 아니라 디렉터리를 준다** ([`quantization.md`](quantization.md) §2) |
...
| (VRAM 자동) | `--gpu-memory-utilization 0.9` | [`quantization.md`](quantization.md) §1.1 |
...
### Step 1 — 첫 기동 ([`quantization.md`](quantization.md) §3 기준으로 8B급 AWQ)
```bash
...
```
- llama-server 가 떠 있으면 util 을 낮추거나 먼저 내린다 ([`quantization.md`](quantization.md) §1.1)
...
> **실측 결과는 [`performance.md`](performance.md) 로 옮겼다.**
...
> **실측 결과는 [`performance.md`](performance.md) §7 로 옮겼다.**
> 요약: vLLM 이 프리필 2.0배 · 디코딩 1.2~**4.4배** 우세하고 동시성이 오를수록 격차가
> 벌어진다. llama.cpp 는 `--parallel N` 슬롯에서 확장이 멈춘다. 진짜 차이는 처리량이
> 아니라 **메모리 모델**이다 — llama.cpp 는 `-c` 를 N 등분해 고정하고 초과 요청을 거부한다.
...
6. **util 은 전체 기준** — 다른 프로세스 점유를 빼서 계산 ([`quantization.md`](quantization.md) §1.1)
